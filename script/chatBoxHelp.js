...
//编码表情
function formatContentWithImage(msg){
    if(isBlack(msg)){return ''}
    var formatContent = msg.replace(/\[(.+?)\]/g,
        function(word){
...
function getImagePathFormString(string){
    for(var i=0;i<chatImage.length;i++){
        if(chatImage[i].text==string){
            return "../image/emotion/"+chatImage[i].name+".png";
        }
    }
    return string;
}

function getStringFormImagePath(path){
    for(var i=0;i<chatImage.length;i++){
        if(path.indexOf(chatImage[i].name)!=-1){
            return chatImage[i].text;
        }
    }
    return path;
}

function transImageToText(content){
    content = content.replace(/<img class="iconImg" src="(.+?)">/g,
        function(word){
            var text=getStringFormImagePath(word);
            return text;
        });
...
//content,voice,pictures
function getContent(authorId,msgContent,encode){
    var maxWidth=Math.round(api.frameWidth*0.5);
    if(maxWidth>200){
        maxWidth=200;
    }

    var formatContent;
    //alert(JSON.stringify(msgContent.messageChat));
    if(isNotBlack(msgContent.messageChat.voice)){
        //alert("voice")
        var voice=msgContent.messageChat.voice;
        var duration = msgContent.messageChat.voiceDuration;
        //alert("语音"+voice)
        var user=getUserInfo();
        if((isNotBlack(authorId)&&user.id==authorId)){
            formatContent = '<img alt="98" style="margin-right: 50px;" class="voiceImg" ' +
                'data-path="'+voice+'" data-type="0"' +
                'src="../image/msendlog.png"/>'+duration+'"';
        }else{
            formatContent = duration+'"<img alt="97" style="margin-left: 50px;"  class="voiceImg" ' +
                'data-path="'+voice+'" data-type="1" ' +
                'src="../image/mrecelog.png"/>';
        }

    }else if(isNotBlack(msgContent.messageChat.picture)){
        var img=msgContent.messageChat.picture;
        //alert(JSON.stringify(img));
        if(isNotBlack(img.thumbUrl)){
            var newHeight= Math.round(maxWidth/img.w*img.h);
            formatContent = '<img class="commonImg" src="' + img.thumbUrl + '" width="'+maxWidth+'px" height="'+newHeight+'px" real="'+img.url+'"/>';
        }else{
            var thumbUrl=getNewImageUrl(img,200,0);
            formatContent = '<img class="commonImg" src="' + thumbUrl + '" width="'+maxWidth+'px" height="'+getImageHeight(thumbUrl,maxWidth)+'px" real="'+img+'"/>';

        }
        //alert(formatContent)
    }else if(isNotBlack(msgContent.messageChat.word)){
        var newContent=msgContent.messageChat.word;
        if(encode){
            newContent=html_encode(newContent);
        }
        formatContent =newContent.replace(/\[(.+?)\]/g,
            function(word){
                var path=getImagePathFormString(word);
                return '<img class="iconImg" src="'+path+'">';
            });
...
function getImageHeight(url,thumbWidth){
    //alert(url)
    if(url.startWith("http")){
        //var linkRegx = /http:\/\/([^\/]+)\/youhuipai\/(\d+)\*(\d+)\*/;
        var s1=url.split("/fangpai/");
        var group=s1[1].split("*");
        ///var group = url.match(linkRegx);
        var orgWidth=parseFloat(group[0]);
        var orgHeight=parseFloat(group[1]);
        return Math.round(thumbWidth/orgWidth*orgHeight);
    }else{
        return 400;//默认
    }
}

function getNewImageUrl(url,width,height){
    var src = url.substring(0,url.lastIndexOf("."));
    var suffix = url.substring(url.lastIndexOf("."));
    //var height = width/h;
    src = src+"_"+width+"_"+height+suffix;
    return src;
}