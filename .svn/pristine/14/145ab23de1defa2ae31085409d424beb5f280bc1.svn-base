...
function _initSystemInfo(callback){
    ajaxGet(initUrl,{},function(ret,err) {
        ...
        //  	alert(JSON.stringify(ret));
            ...
            var enterpriseHousesTypes =ret.enterpriseHousesTypes;
            ...
            createEnterpriseHousesTypes(enterpriseHousesTypes);
...
function createEnterpriseHousesTypes(list){
    var types = {};
    if(list && list.length>0){
        for(var i = 0;i< list.length > 0;i++){
            types[list[i].id] = list[i];
        ...
        $api.setStorage("createEnterpriseHousesTypes",types);//楼盘状态
        $api.setStorage("createEnterpriseHousesTypesList",list);//楼盘状态数组
    }
