            ...
            if(mySendMsgList.length>0){
                for(var i=mySendMsgList.length-1;i>0;i--){
                    ...
                    if(mySendMsgList[i].type!=type||mySendMsgList[i].thirdId!=thirdId){
                        if(myMsg.status=='ok'&&nowTime-myMsg.msg.time>3600000){
...
function uploadAndSendMsg(type,thirdId,msg,callBackWhenLocal,callBackWhenSuccess,callBackWhenError,addToMySendList){
    ...
    if(isNotBlack(msgContain.pictures)){
        var picture=JSON.parse(msgContain.pictures);
        if(addToMySendList){
        ...
        compressImage(picture.size||0,0,picture.url,function(ret){
            ...
            uploadFile(ret.path,function(ret){
                if(ret.status){
                    msgContain.pictures=JSON.stringify({url:ret.result});
    ...
    }else if(isNotBlack(msgContain.voice)){
        var voice=JSON.parse(msgContain.voice);
        if(addToMySendList) {
            addMySendMsg({type: type, thirdId: thirdId, msg: msg, tempFile: [voice.path]});
        }
        uploadFile(voice.path,function(ret){
            //alert("uploadVoice"+JSON.stringify(ret));
            if(ret.status){
                voice.path=ret.result;
                msgContain.voice=JSON.stringify(voice);
                //msg.message=msgContain;
                _sendMsgToServer(type,thirdId,msg,callBackWhenLocal,callBackWhenSuccess,callBackWhenError);
            }else{
                callBackWhenError(msg);
...
function _sendMsgToServer(type,thirdId,msg,callBackWhenLocal,callBackWhenSuccess,callBackWhenError){
    var msgContain=msg.message;
    var getData={clientId:msg.clientId,type:type,thirdId:thirdId,content:msgContain.content,voice:msgContain.voice,pictures:msgContain.pictures};
...
//	            			alert(JSON.stringify(ids));
            ...
            if(ret.streams.length > 0){
                _message.saveMessage(ret.streams,function(){
                    ...
                    _message.setIndex(ret.maxIndex);
