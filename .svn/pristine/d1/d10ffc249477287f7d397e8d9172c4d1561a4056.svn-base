...
(function($, document) {
	...
	<div class="mui-poppicker-title" ></div>\
		<div class="mui-poppicker-bottom-button" >\
			<button class="mui-poppicker-bottom-left mui-poppicker-btn-cancel">取消</button>\
			<button class="mui-poppicker-bottom-right mui-poppicker-btn-ok">确定</button>\
	...
	var PopPicker = $.PopPicker = $.Class.extend({
		...
		_createPicker: function() {
			...
			for (var i = 1; i <= layer; i++) {
				...
				var rule = pickerElement.querySelectorAll(".mui-pciker-rule");
				var list = pickerElement.querySelectorAll(".mui-pciker-list");
				...
				if(layer>1){
					for(var j = 0;j<list.length; j++){
						list[j].style.width = "70%";
						list[j].style.marginLeft = "15%";
					}

					for(var j = 0;j<rule.length; j++){
						rule[j].style.width = "70%";
						rule[j].style.marginLeft = "15%";
					}
				}
		...
		_changePosition: function () {
			var self = this;
			if(self.options.layer && self.options.layer == 2){
				self.position.style.marginLeft="15%";
				self.position.style.width = "70%";
			}
